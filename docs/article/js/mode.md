...
## 概念
设计模式是一种被反复使用的、多数人知晓并且经过分类的代码设计经验的总结。当然项目中可能不会见到太多的设计模式相关代码，它更多的被应用在设计框架、类库、插件中，这也说明了使用设计模式来实现的代码更加的能够使他人理解、保证代码的可靠性。

所以，有必要去了解一些常见的设计模式。

## 设计原则
在使用设计模式编写代码的时候，要遵守一定的原则
...
2.  使用 IIFE创建局部作用域并即时执行；
    
3.  `getInstance()`为一个 闭包 ，使用闭包保存局部作用域中的单例对象并返回。
...
```js
function Member(name) { this.name = name; this.chatroom = null; } Member.prototype = {
    // 发送消息  
...
```js
function Chatroom() { this.members = {}; } Chatroom.prototype = {
    // 增加成员  
...
bruce.send('Hey frank', frank);//输出：bruce to frank: hello frank
```
